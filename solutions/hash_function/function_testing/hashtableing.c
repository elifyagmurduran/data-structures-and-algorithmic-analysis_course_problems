...
void insert(char str[], char docname[]) {
    item *hashitem = find(str);
    if (hashitem != NULL) {
        hashitem->docs = strcat(hashitem->docs, ",");
        hashitem->docs = strcat(hashitem->docs, docname);
        return;
    }
    ...
    hashitem->key = key;
    strcpy(hashitem->docs, docname);

    while (table[j] != NULL && table[j]->key != -1 && i <= M) {
        j = hashDouble(key, i);
        i++;
    }
