...
int main() {
    ...
    for (i = 2; i < 7 + 1; i++) { // profitte gezen indis
        j = i - 1;
        k = i - 2; // table da j profitte i nin oldugu harfi, k bir �nceki harfi g�sterdi
        while ((k >= 0) && (table[k].endtime > table[j].start)) {
            k--;
        }
        //�akisma olmayan son reklam bulundu
        if ((profit[k + 1].revenue + table[j].value) > profit[i - 1].revenue) {
            // reklami eklemeyi se�tik, o zaman profit ini hesapla
